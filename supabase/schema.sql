...
create table if not exists credit_card_invoices (
  ...
  -- Saídas do restante, pagas depois (20260916200000_pagar_restante_fatura.sql).
  extra_transaction_ids uuid[] not null default '{}',
  created_at timestamptz not null default now(),
...
-- Idempotência da fila offline (24/09/2026): migration
-- 20260924230000_idempotencia_fila_offline.sql. Chave gerada pelo aparelho,
-- única por usuário; NULL (APK antigo) não colide. Índice não parcial pelo
-- mesmo motivo de `fitid`.
...
-- ============================================================================
-- Push remoto dos lembretes de hábito — 04/09/2026
--
-- O token fica ligado ao aparelho, não a uma instalação específica do app.
-- A Edge Function cria uma entrega por dia local e consulta o recibo do Expo;
-- `DeviceNotRegistered` remove o token depois da desinstalação. A tabela de
-- entregas é uma outbox: uma queda entre escolher a copy e chamar o Expo não
-- perde o lembrete nem sorteia outro texto no retry.
-- ============================================================================
...
-- O contexto vem em lote para a function não fazer uma consulta por usuário.
-- Só service_role pode chamar: as datas financeiras não são expostas pelo RPC.
...
-- Reserva com SKIP LOCKED para duas execuções sobrepostas do cron nunca
-- enviarem a mesma linha. `sending` vence em 15 minutos: se a function cair
-- depois da reserva, outra execução recupera a entrega automaticamente.
