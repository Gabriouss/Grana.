...
// não importa nada de lib/ do projeto Expo: as heurísticas de valor/descrição
// abaixo são uma cópia mínima de lib/heuristics.ts, mantidas em sincronia à
// mão (RN não roda em Deno e vice-versa, então um pacote compartilhado ali
// não é possível). O dicionário de categoria (CATEGORY_KEYWORDS) É
// compartilhado via ../_shared/category-keywords.ts — Deno importa Deno sem
// esse problema, e passou a servir também assistente-financeiro/index.ts.
//
...
/* CATEGORY_KEYWORDS, normalizarParaBusca e contemPalavra moraram aqui e foram
   extraídas pra supabase/functions/_shared/category-keywords.ts pra serem
