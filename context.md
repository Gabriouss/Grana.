...
**Pendência levantada, não implementada**: `supabase/functions/whatsapp-webhook/index.ts` tem `CATEGORY_KEYWORDS`, um dicionário rico e testado (`__tests__/corpus-consulta.ts`) de sinônimo→categoria ("mercado", "ifood", "uber", "netflix"... → categoria real) pras 9 categorias padrão. Cruzar esse dicionário na resolução de categoria do Granabô (`casarNomeComMemoria`) resolveria sinônimo de forma determinística pra QUALQUER usuário, sem depender do modelo acertar sozinho nem de aprendizado por usuário — mas fica pra decisão do autor numa próxima rodada (envolve duplicar ou compartilhar código entre duas Edge Functions).
