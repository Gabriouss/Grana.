...
**Pendente**: Granabô "ainda com problemas para responder" (queixa do autor, sem novo detalhe) — investigação começada e interrompida por instrução explícita de priorizar o trabalho acima; hipótese líder segue sendo instabilidade transitória do tier grátis do Gemini (já confirmado uma vez: uma falha de 31s seguida de três sucessos de 2-3s). Retomar puxando `function_logs`/`function_edge_logs` frescos (retenção curta — reproduzir e puxar na mesma hora). Build EAS não disparada nesta sessão nem na anterior; quando o autor pedir, cobre 1.5.1 (rótulos/widget/voz/performance) + esta rodada (carteiras).
