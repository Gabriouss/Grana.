...
**Rótulos dos botões (`95a6bb1`, `3465752`).** Escolhidos pelo autor junto da
mudança de destino, porque "Assinar agora" prometia pagamento e o botão leva a
preços. São dois papéis, cada um numa constante: `ROTULO_CTA_DECISAO` ("Já me
decidi. Quero agora") no herói e no fechamento, e `ROTULO_CTA_PLANOS` ("Ver os
planos") na faixa da garantia, onde a pessoa ainda está se tranquilizando. O
card de preço mantém "Assinar o plano anual/mensal", que é o único que cobra.
...
## 24/09/2026 — M1 — T10/T11 no repositório (`e5642dd`), sem QA Android

Relatório do Harbor: `E:\Grana-temporarios\2026-09-24-harbor\relatorio-harbor-t10-t11.md`. Corrige o estado anterior de “árvore local, sem teste”: `e5642dd` foi publicado em `origin/main`.

- **Sintoma:** saída sem rede com sessão antiga girava por mais de quatro minutos, sem recibo ao fechar; a faixa “Atualização pendente” podia permanecer após a rede voltar. **Causa reproduzida no servidor local com supabase-js real:** o `fetch` do cliente não tinha prazo; uma renovação de token pendurada prendia a trava de sessão. A hipótese de conexão keep-alive morta no React Native **não foi medida no Android**.
- **Mudança:** `lib/fetch-com-prazo.ts` põe prazo total de 20 s para banco/auth e 60 s para storage/functions, incluindo a leitura do corpo; `lib/supabase.ts` aplica o prazo por fora do retry de relógio; `__tests__/fetch-com-prazo.cjs` entrou no `test:ci` via `package.json`.
- **Descartado:** proteger só cabeçalhos (deixaria o corpo pendurado), repetir escrita após timeout (risco de duplicar POST) e somar prazos dentro do retry. O limite de uso interrompeu a sessão anterior antes do teste; depois, a árvore compartilhada com T2 ainda em andamento impediu o `test:ci` integral de fechar naquela sessão.
- **Verificação reportada:** teste do módulo real e do cliente Supabase real **29/29**, controle sem prazo pendura, duas mutações derrubam o teste, `tsc --noEmit` limpo; restante da cadeia executado separadamente, incluindo `test:parser`. Sem teste em aparelho/emulador no modo avião. O pior caso estimado de cerca de 40 s ainda é longo para o botão; teto do chamador e recibo “guardado no aparelho” pertencem a T13.

## 24/09/2026 — M1 — T2: Últimos lançamentos atualiza após gravação (`d1cf583`)

Relatório do Forge: `E:\Grana-temporarios\2026-09-24-forge\relatorio-forge-T2.md`. Pedido: depois de salvar fora da Início, “Saldo atual” mudava mas “Últimos lançamentos” só mudava ao trocar de aba. A carga leve no foco não buscava lançamentos; o saldo vinha de contexto compartilhado.

`lib/lancamentos-alterados.ts` (novo) guarda um contador em memória. `lib/data.ts` marca após gravação/edição/exclusão bem-sucedida; `lib/offline-cache.ts` marca ao sincronizar ao menos um item; `app/(app)/index.tsx` faz carga completa no foco se a versão mudou. Três harnesses de teste ganharam dublê para o novo import. O commit `d1cf583` contém apenas esses oito arquivos.

Forge descartou carga completa em todo foco (custo de histórico), parâmetro de rota (cobertura parcial) e consulta leve de `created_at` (não detecta edição/exclusão). A CI primeiro falhou em três dublês de import, corrigidos; um caso novo tinha premissa errada de deduplicação, corrigida no teste. **Verificação reportada:** `tsc` e `test:ci` verdes; `inicio-lancamentos-alterados.cjs` **18/18** e três mutações detectadas. **O teste novo ainda não está no `test:ci`**; Harbor foi liberado para integrar T13 e essa entrada no `package.json`. Sem reverificação no emulador/aparelho. Voz/widget em segundo plano não marca o contador; não foi incluído em T2.

## 24/09/2026 — M1 — Prism publicou T19 e correções visuais; T18 segue aberto

Relatório do Prism: `E:\Grana-temporarios\2026-09-23-QA\relatorio-prism-T19.md`. O código de T19 está publicado em `90357dc`: `AppModal` usa `SafeAreaProvider` para medir a janela nativa sem recuar todo o fundo, e `InsetsDoModal` leva os recuos a Onboarding, QR e retrospectiva mensal. `modais-regressao.cjs` exercita o modal real e falha por mutação se o `SafeAreaView` global voltar. **Mecanismo confirmado no código:** o primeiro `SafeAreaView` global deixava à mostra o fundo branco da janela nativa; `useSafeAreaInsets` lido fora do modal dava topo zero. A solução de pintar o `SafeAreaView` e a de retirar `statusBarTranslucent` foram descartadas por regressões nas folhas e demais modais.

Na mesma rodada: `345abfc` ajusta T1/T8/S10/T6 (layout); `21c80c9` cria `AppDialog` para confirmações de Boletos e voz (T3 parcial; Crédito ainda pendente); `94dfd85` corrige T4/T5/T9/T12, com casos novos em `corpus-sequencia.ts`. O `test:ci` na árvore compartilhada esbarrou em imports ainda não dublados de T2; na cópia isolada passou até uma guarda que exige `.git`, executada verde no repo real **273/273**, com `sync-parser` **75/75**, `tsc` limpo e teste de modais verde. **Nenhuma das correções de `90357dc` foi reverificada no emulador**; T19 aguarda Sentinel. O código no repo mudou; produção e bundle instalado não foram atualizados nesta rodada.

**Novo QA do Sentinel:** T18 foi confirmado ao editar uma categoria em Gerenciar categorias com teclado aberto: Salvar, Cancelar e paleta ficam sob o teclado. Evidência: `E:\Grana-temporarios\prints\qa-categoria-custom-edit.png`, fora do repo/vault. Prism é o dono. Após esse relato, `2f795cb` publicou código e teste de edição de categoria; sem relatório de QA da correção, T18 continua sem reverificação visual. T16/T17 não terem reproduzido na passada anterior não prova resolução.

## 24/09/2026 — M1 — Briefing de voz das notificações recebido; implementação pendente

O Beacon entregou `E:\Grana-temporarios\notificacoes\beacon-voz-e-catalogo.md`, **proposta editorial sem alteração de código**. A inspiração no Duolingo é presença e variedade; tom humano, breve e sem culpa. O briefing distingue sinais conhecidos de mensagens propostas e alerta contra presumir gasto, abertura do app ou sucesso offline. “Dias sem lançamento” não prova inatividade; “guardado no aparelho” só cabe com persistência local confirmada. Ver [[Briefing de Voz para Notificações]] no vault.

**Triagem comunicada pelo Watchtower:** o bug de mensagem de sexta/domingo foi corrigido no código em `9ef9458` (11/09) e o teste dirigido passou **11/11**; a entrega real ainda não passou por QA. O desencontro entre `jaLancouHoje` local e Edge foi confirmado estaticamente; Harbor tem diff e teste em andamento, sem conclusão reportada. `diasInativo` mede tempo desde a última transação, não abertura do app. A copy que afirma gasto, Pix, delivery, almoço ou resultado de operação não dispõe de estado que sustente essas frases. Tom pressionador é decisão editorial. Estes são fatos de código, hipótese/limite de entrega e material de produto separados; **não há resolução ponta a ponta**.

## 24/09/2026 — M1 — T18 reverificado; notificações cliente e Edge no repo, sem entrega ponta a ponta

**T18 fechado no QA do bundle `2f795cb`, segundo o Sentinel.** Com teclado aberto, o formulário de Gerenciar categorias rolou; campo, paleta, Cancelar e Salvar ficaram visíveis. Tocar ciano e Cancelar descartou; reabrir e Salvar fechou sem mudança. Evidência `E:\Grana-temporarios\prints\qa-t18-2f795cb-editor.png` (fora do repo/vault). O commit `2f795cb` corrigiu `CategoryPickerModal.tsx` com `scrollTo` até a edição e `flexShrink: 1`; `categoria-edicao-visivel.cjs` entrou no `test:ci`, `tsc` limpo. A primeira versão do teste selecionava o invólucro errado e comparava objetos de contextos `vm` diferentes; foi corrigida no teste. Mover o formulário para fora da lista foi descartado para preservar o vínculo visual. O texto mojibake “SaÃºde/SalÃ¡rio/AlimentaÃ§Ã£o” no print **não é bug confirmado**: não há fonte no código; dado externo é hipótese a investigar separadamente. A categoria de teste AUDIT será removida pelo Sentinel ao fim da rodada.

**Handler Edge, `4743718` (Harbor):** com push remoto ativo, o app cancela o agendamento local; o servidor podia criar ou enviar lembrete mesmo após lançamento no dia. O código novo evita criar entrega quando há `occurred_on` no dia local e marca retentativa posterior como `failed/already_logged_today`. `lembretes-habito-handler.cjs` exercita handler e módulos reais com banco/Expo simulados; o commit relata **1.143 checagens**, mutações nas guardas e inclusão do teste no `test:ci`. Também incluiu `inicio-lancamentos-alterados.cjs` de T2 no `test:ci`. **Sem deploy da Edge**; produção permanece na v12 verificada em 23/09 até novo preflight da regra 11.

**Cliente, `22333aa` (Forge):** relatório em `E:\Grana-temporarios\2026-09-24-forge\relatorio-forge-notificacoes.md`. `lib/contexto-lembrete.ts` unifica Início, Perfil e layout usando dia de registro `created_at` local (com fallback para `occurred_on`) para `jaLancouHoje`/`diasInativo`. Parcela futura já não torna a inatividade negativa nem silencia indevidamente o lembrete. `lembreteAindaFiel` reavalia apenas mensagens agendadas que afirmam inatividade ou sequência; vencimento de fatura passa a usar `lib/faturaCiclo.ts` e respeita fim do mês. Arquivos: `lib/contexto-lembrete.ts` (novo), `lib/notifications.ts`, `app/(app)/index.tsx`, `app/(app)/perfil.tsx`, `app/_layout.tsx` e `__tests__/notificacoes-estado-real.cjs`. Forge reportou teste dirigido **14/14**, mutações **2/2**, `notificacao-dia-certo` **11/11**, corpus de notificações **37/37**, `tsc` limpo e a cadeia atual de `test:ci` verde. O teste novo **não consta ainda em `test:ci`**; a CI ainda não cobre automaticamente N1/N2/N3. `inicio-lancamentos-alterados.cjs` de T2 **já entrou** no `test:ci` em `4743718`; a pendência citada no relatório de T2 foi resolvida. Catálogo/copy e Edge não mudaram em `22333aa`.

**Paridade e limites:** a leitura dos dois commits mostra que o cliente agora mede atividade por `created_at`, enquanto o handler `4743718` verifica `occurred_on`. `contextoDasDatas` da Edge ainda usa `Math.max(0, ...)`, que pode mascarar data futura como inatividade zero. A decisão app/push para registro retroativo ou parcela futura não foi provada equivalente; requer reconciliação e teste conjunto. Forge descartou duplicar a prioridade do seletor ou ressortear toda a janela de sete dias; um teste inicialmente pressupôs retomada em toda a janela, mas o catálogo esgota oito mensagens e cai no pool geral, então a premissa do teste foi corrigida. O QA de entrega real de sexta/domingo, push, agendamento local e exibição no aparelho continua pendente (Expo Go não exercita notificações nativas). `22333aa` está no repositório, mas ainda não em novo bundle; `4743718` não está em produção. Harbor iniciou S43 no backend após T13/T2, sem relatório de resultado: não declarar corrigido.

## 24/09/2026 — M1 — S43/A46 no repo, sem deploy (`fe8e210`)

Harbor publicou `fe8e210` em `origin/main` com quatro arquivos: `supabase/functions/_shared/caixa.ts` (novo), `assistente-financeiro/index.ts`, `__tests__/granabo-caixa-sem-credito.cjs` (novo) e `package.json` (teste no `test:ci`). Sintoma confirmado pelo Sentinel: “quanto gastei em alimentação” devolvia 486,41 no Granabô contra 141,41 em Gráficos, pois `gastoPorCategoria` incluía compras no crédito no gasto de caixa. `resumoMes` e `livreParaGastar` tinham a mesma regra. A correção aplica a classificação de `isCreditTx` do app (`payment_method = credit` ou `card_id`) às três ferramentas, sem alterar modo fatura/`resumoCredito`; pagamento da fatura continua no caixa e estorno de cartão fica fora. O filtro é em memória para não perder compra com cartão e forma de pagamento nula.

Relatório do Harbor: `E:\Grana-temporarios\2026-09-24-harbor\relatorio-harbor-s43-notificacoes.md`. **Causa confirmada no código:** o `5d72611` só tinha acrescentado `AVISO_REGRA_CREDITO` ao texto, sem mudar a soma. `ehCompraNoCredito` em `_shared/caixa.ts` é cópia de `isCreditTx` (`lib/transaction-rules.ts`), porque a função Deno não importa `lib/`; a igualdade das duas é coberta pelo teste. Descartados: filtro PostgREST `neq('payment_method','credit')` (perde compra de cartão com forma nula) e mexer em `consultarLancamentos` (filtro explícito do usuário), modo fatura e `resumoCredito`. O commit ficou local alguns minutos porque o `pull --rebase` recusou árvore suja de outros agentes (sem stash, regra 10).

**Reportado pelo Harbor:** **39/39** (28 combinações de paridade e handler real nas três ferramentas com o caso do Sentinel), três mutações derrubando, `deno check`, `tsc` e `test:ci` verdes. **Reexecutado pelo Ledger em 24/09:** `granabo-caixa-sem-credito.cjs` **39 OK**. **Nenhum deploy:** a Edge em produção segue na v37 anterior à correção, e o resultado S43 não foi reverificado em produção nem no Granabô real.

## 24/09/2026 — M1 — Paridade app x push dos lembretes (`bb019dd`), sem deploy

Relatório do Harbor: o mesmo arquivo da seção anterior. Fecha a divergência registrada acima entre o cliente (`created_at` no dia local, `22333aa`) e o handler de `4743718` (`occurred_on`).

- **Sintoma:** gasto de ontem registrado hoje silenciava o lembrete local mas não o push; parcela de hoje criada antes silenciava o push mas não o local; `contextoDasDatas` fazia `Math.max` com datas futuras, e a retomada nunca saía para quem tem parcela futura.
- **Mudança:** `enviar-lembretes-habito/index.ts` lê `user_id`, `created_at` e `occurred_on` direto de `transactions` (service role, 60 dias, paginado) em vez da RPC `contextos_push_habito`, que fica no banco sem uso. `_shared/push-habit.ts` ganha `lancouNoDia` e `diasDeAtividade` (`created_at` no fuso do token; `occurred_on` só quando falta `created_at`) e corta datas futuras. Testes novos: `lembrete-paridade-servidor.cjs` (módulo real do app contra o módulo real do servidor) e casos novos em `lembretes-habito-handler.cjs`; `test:ci` passa a rodar `notificacoes-estado-real.cjs` e a paridade.
- **Descartado:** migration trocando a RPC para `created_at` (exigiria aplicar em produção, e a RPC não sabe o fuso de cada token); editar `lib/notification-catalog.ts` (compartilhado, do Forge/Beacon).
- **"Domingo na sexta":** o código atual impede, segundo o Harbor, provado no handler real perto da meia-noite em São Paulo e Manaus. A causa histórica apontada (catálogo com filtro por dia de 11/09, `9ef9458`, contra função publicada de 10/09 até o deploy da M2 em 23/09) é **inferência pelas datas**, não pelo pacote publicado antigo.
- **Deu errado:** `4743718` saiu com `occurred_on`, trocado no mesmo dia; um script via `python` travou no atalho da Microsoft Store e foi abortado sem efeito.
- **Reportado pelo Harbor:** handler **1564** checagens, paridade **4015** (retroativo, futuro, sem `created_at`, perto da meia-noite, 4000 históricos gerados), quatro mutações derrubando, `deno check`, `tsc` e `test:ci` verdes. **Reexecutado pelo Ledger em 24/09:** `lembretes-habito-handler.cjs` **1564 OK**, `lembrete-paridade-servidor.cjs` **4015 OK**, `notificacoes-estado-real.cjs` **14/14**, na árvore de trabalho da M1 (que tinha mudanças não commitadas de outro agente em `lib/`). Ambos constam do `test:ci` do `package.json`.
- **Não verificado:** entrega real por FCM, produção, e sequência maior que 60 dias truncada pela janela de leitura.
- **Preflight da regra 11 (só leitura, ~19h45, Harbor):** `assistente-financeiro` v37, `updated_at` 2026-09-23T21:06:05Z, `verify_jwt=true`; `enviar-lembretes-habito` v12, `updated_at` 2026-09-23T21:06:12Z, `verify_jwt=false`. Sem sinal de drift. Aguardam deploy: `1bfae42`, `37318e5`, `d10a79a`, `a249e96`, `fe8e210` (assistente) e `4743718`, `bb019dd`, `c577258` (lembretes, que importa o catálogo). No momento do deploy faltam: baixar o `/body` de cada uma, `deno check` final e publicar cada função nomeada com o `verify_jwt` atual. **Deploy depende do OK do autor.**

## 24/09/2026 — M1 — Copy dos lembretes de hábito (`c577258`), sem entrega no aparelho

Relatório do Forge: `E:\Grana-temporarios\2026-09-24-forge\relatorio-forge-copy-notificacoes.md`; textos finais em `copy-notificacoes.json`, na mesma pasta. Pedido: copy humana, curta e sem culpa, sem afirmar gasto, abertura do app, Pix, delivery, almoço ou sucesso offline, pelo briefing do Beacon.

- **Causa:** o seletor só conhece dias sem registrar, dia da semana, sequência e janela; a copy tinha sido escrita como se soubesse mais ("Aquele Pix de R$ 3", "a gente não se vê", "10 segundos").
- **Mudança:** só `lib/notification-catalog.ts`, título e corpo de **37 ids** (noturno, streak, micro, finde, saudade, dica e almoço 1 a 9). Ids, `dias`, categoria, `selecionarMensagem`, pools e agendamento intactos, para `mensagens_recentes` continuar rodando. O comentário da categoria saudade passou a dizer "dias sem registrar". Teste novo `__tests__/catalogo-notificacoes-copy.cjs`: texto aprovado por id, régua no catálogo inteiro (título até 32, corpo até 90, sem gasto, cobrança, promessa ou travessão), alcance por dia e janela, e fallback com categoria esgotada; falha no catálogo anterior.
- **Descartado:** trocar ids ou criar mensagens novas (quebraria a rotação); tirar o fim de semana da janela de almoço de sexta (mexe no seletor compartilhado com a Edge; decisão aberta ao autor/Beacon); copy de conta, fatura e limite do cartão em `notifications.ts` (rodada própria).
- **Reportado pelo Forge:** `notificacao-dia-certo` 11/11, `corpus-notificacoes` 37/37, `notificacoes-estado-real` 14/14, handler 1564, `tsc` limpo, `test:ci` verde. **Reexecutado pelo Ledger:** `catalogo-notificacoes-copy.cjs` **4 checagens, 0 falhas**. **Esse teste NÃO está no `test:ci`** (conferido no `package.json` em 24/09); a inclusão foi pedida ao Harbor.
- **Não verificado:** exibição no aparelho e na tela bloqueada (Expo Go não tem o módulo). O push só leva o texto novo depois do deploy autorizado do `enviar-lembretes-habito`; o local, na próxima build.

**Revisão do Beacon** (`E:\Grana-temporarios\2026-09-24-beacon\revisao-copy-notificacoes-c577258.md`, só texto): **aprovado com ajustes finos, sem bloqueio.** Prioridade 1, fidelidade ao briefing: `dica-5` ("preenche" promete automação), `almoco-2` ("teve" afirma um almoço que o app não viu), `almoco-9` (pressupõe gastos da manhã). Prioridade 2, repetição: "também conta/somam" em cinco títulos, "cabe no Grana." em quatro corpos, cerca de quinze aberturas com "Se". Fora dos 37, para a próxima rodada: `streak-7`, `dica-2`, `saudade-2` (perto do "não é X, é Y"), `streak-6`, `micro-8`, `saudade-8`. **Nenhum ajuste aplicado ainda.** O parecer diz que os ajustes "não bloqueiam o que já está em produção", mas `c577258` **não está em produção**: nem bundle nem Edge publicados.

**Pendente:** relatório da retomada do Sentinel (`E:\Grana-temporarios\2026-09-23-QA\relatorio-sentinel-retomada-24-09.md`) ainda não existia quando esta entrada foi escrita.

## 24/09/2026 — M1 — T3 fechado no Crédito (`50dfdda`), sem QA no aparelho

Sem relatório próprio; registrado pelo `git show`. "Desfazer pagamento" da fatura era o último dos três alertas nativos da T3 (cinza, CANCELAR/DESFAZER em caixa alta); Boletos e voz já tinham ido em `21c80c9`, e o Crédito ficou para depois porque o Forge fechava o crédito por ciclo no mesmo arquivo. `app/(app)/credito.tsx`: `confirmReopenInvoice` só guarda o pagamento e abre o `AppDialog` destrutivo "Desfazer pagamento?"; a ação foi para `desfazerPagamento`, com o mesmo corpo de antes. Teste novo `__tests__/app-dialog.cjs` (no `test:ci`) executa o `AppDialog` real e confere as três telas da T3; falha com o `credito.tsx` anterior.

**Deu errado:** o `50dfdda` levou no `package.json` compartilhado a linha do `test:ci` do Harbor (`offline-pendente-na-lista.cjs`) antes de o arquivo existir no git; o HEAD ficou quebrado ~2 minutos, até o `03534c8`. Desde então os agentes montam o `package.json` do commit a partir do HEAD, só com a própria linha. **Reexecutado pelo Ledger no HEAD `5be1508`:** `app-dialog.cjs` OK. **Sem verificação:** emulador/aparelho.

## 24/09/2026 — M1 — T13: lançamento salvo sem rede aparece na lista e sobe sozinho (`03534c8` + `5be1508`), só no repositório

**Pedido:** achado T13 do Sentinel (P2), núcleo com o Harbor e troca nas telas com o Forge. **Sintoma:** sem rede, app aberto do zero, o lançamento fechava a folha, mas não aparecia na lista nem nos totais, e só sincronizava ao reabrir o app (regressão do A59, visto certo em 22/09). **Causa, confirmada no código e reproduzida no teste com módulos reais:** `fetchTransactionsDoPeriodo` sai por `comCacheOffline`, que sem rede devolve o mês do disco como sucesso; o disco não conhece o pendente, e o `guardarNoCache` de Lançamentos ainda apagava o otimista. A fila só era enviada dentro do `load()` de uma tela.

**Núcleo, `03534c8` (Harbor, `relatorio-harbor-t13.md`):** `lib/fila-pendente.ts` (novo, módulo-folha: fila, dono, otimista, `juntarPendentes`); `lib/data.ts` junta os pendentes em `fetchTransactions` e `fetchTransactionsDoPeriodo` depois do cache; `lib/offline-cache.ts` ganha `salvarOuGuardarNoAparelho` (salva; sem rede guarda; recusa do banco sobe), nova tentativa a cada 30 s enquanto houver fila, com `avisarDadoNovo` e `marcarLancamentosAlterados` quando sobe, e log no `catch` que não é de rede; `lib/cache-de-tela.ts` exporta `avisarDadoNovo`. Descartado: prazo próprio no helper (`transactions` não tem chave de idempotência; desistir com o primeiro envio a caminho gravaria duas vezes, o teto é o do T10), juntar dentro de `comCacheOffline` (o disco passaria a guardar otimistas) e pôr a junção em `offline-cache.ts` (ciclo de import com `data.ts`). Deu errado: a primeira versão do teste não pegava a mutação "sem reagendar"; corrigida. Reportado: `offline-pendente-na-lista.cjs` 23/23, mutações "sem juntar" e "sem reagendar" derrubam, `tsc` e `test:ci` verdes.

**Telas, `5be1508` (Forge, `relatorio-forge-T13-cliente.md`):** Início e Lançamentos trocam o `try { addTransaction } catch { queuePendingTransaction }` por `salvarOuGuardarNoAparelho`; o toast "Sem conexão. Lançamento salvo no aparelho" só aparece com `guardado: true`, que só vem depois de `queuePendingTransaction`. **Defeito achado na troca:** enfileirar não passa por `addTransaction`, então nada marcava `lib/lancamentos-alterados` (T2); salvando sem rede pelo "+" da Início, a volta caía na carga leve e o item não aparecia lá. Lançamentos agora chama `marcarLancamentosAlterados` quando guarda. Descartado: pôr a marca dentro do helper ou de `queuePendingTransaction`, por ser arquivo do Harbor. Teste novo `t13-telas-salvar-ou-guardar.cjs` (7 checagens, mutação 2/2) no `test:ci`. **Conferido pelo maestro no diff e pelo Ledger no código** (`index.tsx:855`, `lancamentos.tsx:496`, `offline-cache.ts:281-286`).

**Hipótese, sem defeito comprovado (maestro):** a marca ficou na tela, não no núcleo; um chamador futuro da fila teria o mesmo buraco. Limites que o Harbor registrou e continuam: janela curta de item duplicado numa recarga concorrente (sem id comum entre otimista e linha real) e item recusado de forma permanente parado na frente da fila (agora com log).

**Reexecutado pelo Ledger no HEAD `5be1508`, árvore limpa:** `offline-pendente-na-lista.cjs` 23 OK, `t13-telas-salvar-ou-guardar.cjs` 7/0; ambos no `test:ci`. **Sem verificação:** emulador/aparelho (Sentinel: "+" da Início sem rede, item na Início e em Lançamentos, subida ao voltar a rede com o app aberto). Sem build, sem deploy.

**Pergunta ao autor, não é defeito resolvido:** a fala de voz pendente sem rede (fila própria em `lib/voice-operations.ts`, idempotente por `request_id`) **não entra em `juntarPendentes`**, então não aparece na lista nem nos totais até subir, no app e no widget (mesma classe do T13; correção teria de ir no núcleo com teste nas duas entradas, regra 13). E o comprovante colado sem voz (`components/PasteReceiptModal.tsx:208`) e o leitor de QR (`components/QrScannerModal.tsx:138`) chamam `addTransaction` direto, sem fila: sem rede aparece "Erro ao salvar", o formulário continua na tela e nada se perde, mas nada é guardado no aparelho. Trocar a voz pelo helper foi descartado porque perderia a idempotência.

## 24/09/2026 — M1 — Copy dos lembretes, segunda rodada (`4902af2`) e teste no `test:ci` (`a3f8806`)

Relatório: `E:\Grana-temporarios\2026-09-24-forge\relatorio-forge-copy-notificacoes-2.md`; textos em `copy-notificacoes-2.json`. **Pedido:** aplicar as prioridades 1 e 2 do parecer do Beacon e, no mesmo commit, as trocas W1-W3 do Watchtower (CDC). **Causa:** copy escrita além do que o seletor sabe. **Mudança:** 14 ids, só título e texto em `lib/notification-catalog.ts`: P1 `dica-5`, `almoco-2`, `almoco-9`; P2 `micro-3`, `micro-6`, `micro-7`, `almoco-4`, `noturno-5`, `noturno-2`, `noturno-4`, `streak-8`; W1-W3 `streak-7` (recorde que o app não sabe), `streak-6` (contagem regressiva), `saudade-2` (culpa e "não é X, mas Y"). `REESCRITAS` no teste: 11 trocados, 3 novos. **Descartado por ordem do maestro:** prioridade 3 (`noturno-1`, `micro-2`, `almoco-1`, `almoco-6`) e `dica-2`, `micro-8`, `saudade-8`, que seguem no parecer. **Deu errado:** nada, segundo o Forge; o acréscimo W1-W3 chegou antes do commit. **Reexecutado pelo Ledger:** `catalogo-notificacoes-copy.cjs` 4/0, handler 1564 OK com o catálogo novo.
...
- **Branch aberta por decisão do autor:** em 26/09/2026 o autor decidiu não juntar nem descartar a `claude/cool-einstein-c63bq0` nesta sessão ("Deixa quieto"). Isso é exceção à regra 10. O PR https://github.com/Gabriouss/Grana./pull/5 continua aberto, e nada dele está na `main`: correções de documentação, paridade de voz com testes e material de marketing. A próxima sessão deve perguntar ao autor o que fazer com ele.
