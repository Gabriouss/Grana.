...
/**
 * Catálogo de mensagens do lembrete diário de hábito. Notificação local
 * agendada tem conteúdo fixo no momento do agendamento — a "rotação" vem de
 * escolher uma mensagem nova cada vez que o app reagenda (ver
 * scheduleDailyHabitReminder em lib/notifications.ts), não de sortear em
 * tempo real no disparo.
 */
...
async function registrarRecente(id: string, recentes: string[]): Promise<void> {
  const atualizada = [...recentes.filter((r) => r !== id), id].slice(-MAX_RECENTES);
  await AsyncStorage.setItem(CHAVE_RECENTES, JSON.stringify(atualizada));
}

function sortear(lista: MensagemNotif[]): MensagemNotif {
...
export async function obterProximaMensagem(contexto: {
  ...
  const escolhida = sortear(candidatas);
